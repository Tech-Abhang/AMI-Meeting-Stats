...
LOCKED-IN SPEC (from your last message):

  1. Eval group = 100 of 190 speakers: 70 male + 30 female, drawn
     randomly (seed 42, reproducible) from the full pool of 130 male /
     60 female speakers. The other 90 speakers (60 male + 30 female) go
     into a separate "train group" speaker list — NOT processed further,
     per your standing instruction to defer train.

  2. ENROLL: near-field (ihm) ONLY. ALL 70 eval-male speakers.
     ALL their meetings, ALL their audio files (no filtering by meeting
     or duration) — every ihm utterance they have, anywhere in the
     corpus (train/dev/eval folders combined).

  3. TEST: near-field AND far-field (ihm + sdm). ALL 30 eval-female
     speakers. Same rule — every utterance they have in both mic
     folders, all meetings, no filtering.

  Because enroll draws only from males and test only from females, the
  two speaker pools never overlap — so there's no meeting-leakage risk
  between a speaker's own enroll and test data (they're different
  people). That's a change from the earlier per-speaker meeting-split
  version of this script; this one is simpler because your gender rule
  makes that unnecessary.

  NOTE (flagging, not blocking): this makes every enroll/test trial in
  this eval set a cross-gender comparison by construction — a genuine
  same-speaker trial is impossible here since no speaker is in both
  pools. If your downstream use needs genuine speaker-match trials
  later, that'll need a different pool. Proceeding with your spec as
  given.

Output (.xlsx, written next to this script unless --out-dir is given):
  Sheet "Enroll"   — one row per ihm utterance, male eval speakers only
  Sheet "Test"     — one row per ihm+sdm utterance, female eval speakers only
...
def main():
    ...
    enroll_rows = [to_row(r) for r in ihm_rows if r["speaker_id"] in eval_male_set]
    test_rows = [to_row(r) for r in all_rows if r["speaker_id"] in eval_female_set]
    ...
    rows_to_write = [
        ...
        ("", None, False),
        ("Notes", None, True),
        ("Enroll = near-field (ihm) only, all eval-male speakers, all meetings/all files.", None, False),
        ("Test = near-field + far-field (ihm+sdm), all eval-female speakers, all meetings/all files.", None, False),
        ("Enroll and Test speaker pools never overlap (disjoint by gender) — see script docstring", None, False),
        ("for why this makes same-speaker verification trials impossible in this eval set.", None, False),
